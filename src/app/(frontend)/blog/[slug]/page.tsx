            ...
            <main className="pt-32 pb-24">
                <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <header className="mb-16 text-center">
                        ...
                        <div className="text-sm font-black text-brand-orange uppercase mb-6 tracking-[0.2em] flex items-center justify-center gap-4">
                            <span>Primary Strategic Intel</span>
                            <span className="w-1.5 h-1.5 bg-brand-neutral/40 rounded-full" />
                        ...
                        <div className="flex flex-col md:flex-row items-center justify-center gap-6 text-gray-500 font-bold border-y border-brand-neutral/30 py-8">
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 bg-brand-blue rounded-full flex items-center justify-center text-white text-sm overflow-hidden border-2 border-brand-blue/10">
                            ...
                            <span className="hidden md:block opacity-30">•</span>
                            <span className="bg-brand-neutral/50 px-3 py-1 rounded text-xs text-brand-blue/70">Proprietary Research</span>
                        </div>
                    </header>

                    <div className="mb-20 rounded-[3rem] overflow-hidden shadow-2xl shadow-brand-blue/10 border border-brand-neutral/20 aspect-[16/9] relative bg-brand-neutral/10">
                        {post.featuredImage?.url ? (
                            ...
                            <div className="absolute inset-0 flex items-center justify-center p-20 bg-gradient-to-br from-brand-blue/5 via-white to-brand-orange/5">
                                <div className="text-brand-blue/10 font-black text-6xl md:text-8xl text-center select-none uppercase tracking-tighter rotate-[-5deg]">
                    ...
                    <div className="prose prose-2xl prose-brand max-w-none text-gray-700 leading-relaxed mb-24">
                        ...
                        */}
                        ...
                        {post.references && post.references.length > 0 && (
                            <section className="mt-20 pt-12 border-t border-brand-neutral/30">
                                <h3 className="text-sm font-black text-brand-orange uppercase tracking-widest mb-8">AI Citations & Authoritative Sources</h3>
                                <div className="grid gap-6">
                                    {post.references.map((ref, i) => (
                                        <div key={i} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-brand-neutral/20 rounded-2xl border border-brand-neutral/50 group hover:border-brand-blue/30 transition-all">
                                            <div>
                                                <h4 className="text-lg font-bold text-brand-blue mb-1">{ref.title}</h4>
                                                <span className="text-xs font-black text-gray-400 uppercase tracking-tight">{ref.sourceName}</span>
                                            </div>
                                            <a
                                                href={ref.url}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="mt-4 md:mt-0 text-brand-orange font-bold text-sm flex items-center gap-2 hover:underline"
                                            >
                                                Verify Source
